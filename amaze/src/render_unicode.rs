...
pub struct UnicodeRenderTable {
    table: Vec<char>,
}

impl UnicodeRenderTable {
    ...
    pub fn lookup(&self, doors: Door4) -> char {
        self.table[*doors as usize]
    }
}

pub fn render_paths_unicode(grid: &Wall4Grid, table: &UnicodeRenderTable) -> String {
    let mut output = String::default();
    for y in 0..grid.height() {
        for x in 0..grid.width() {
            let coord = GridCoord2D::new(x, y);
            let wall: Door4 = !grid[coord];
            output.push(table.lookup(wall));
        }
        output.push('\n');
    }
