...
impl Direction4 {
    ...
    #[inline]
    pub fn include(&mut self, rhs: Self) {
        *self += rhs;
    }
...
impl Debug for Direction4 {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "0b{0:04b} ", self.0).expect("write failed");
        ...
        match *self {
            ...
            dir => {
                ...
                if dir.contains(Self::NORTH) {
                    write!(f, "N").expect("write failed");
                }
                if dir.contains(Self::SOUTH) {
                    write!(f, "S").expect("write failed");
                }
                if dir.contains(Self::EAST) {
                    write!(f, "E").expect("write failed");
                }
                if dir.contains(Self::WEST) {
                    write!(f, "W").expect("write failed");
                }
                write!(f, ")").expect("write failed");
            }
        }
...
mod tests {
    ...
    fn include_combines_directions() {
        ...
        direction.include(Direction4::NORTH);
        direction.include(Direction4::SOUTH);
    ...
    fn remove_removes_directions() {
        let mut direction = Direction4::ALL;
        direction.remove(Direction4::EAST);
        direction.remove(Direction4::WEST);
        assert_eq!(direction, Direction4::NORTH + Direction4::SOUTH);
    }
